...
func (lattigo *LattigoFHE) doPrecisionStats(lineNum int, term *Term, metadata string) {
...
func (lattigo *LattigoFHE) runInstructions(operations []string) ([]*rlwe.Ciphertext, time.Duration, error) {
	...
	for _, line := range operations {
		...
		if lattigo.getStats {
			lattigo.doPrecisionStats(lineNum, term, metadata)
		}
	}
	runtime := time.Since(startTime)

	return results, runtime, nil
}
...
func (lattigo *LattigoFHE) Run() error {
	...
	results, runtime, err := lattigo.runInstructions(operations)
	if err != nil {
